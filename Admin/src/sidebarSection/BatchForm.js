...
function BatchForm({ onAddBatch, onEditBatch, onCancel, isEditMode, batchToEdit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        await axios.put(`${window.location.origin}/api/batches/${batchToEdit._id}`, newBatch);
        onEditBatch({ ...batchToEdit, label: batchName, description: batchDescription });
      } else {
        const response = await axios.post(`${window.location.origin}/api/batches`, newBatch);
